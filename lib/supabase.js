...
const explicitMockMode = process.env.EXPO_PUBLIC_USE_MOCK === 'true' || process.env.NODE_ENV === 'test';
const hasSupabaseEnv = !!(supabaseUrl && supabaseKey);

if (!hasSupabaseEnv && !explicitMockMode) {
  throw new Error(
    'Supabase environment variables are missing. Set EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY, or set EXPO_PUBLIC_USE_MOCK=true for local mock mode.'
  );
}

export const USE_REAL_SUPABASE = hasSupabaseEnv && !explicitMockMode;
