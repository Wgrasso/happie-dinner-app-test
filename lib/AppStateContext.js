...
import { loadRecipeExtras } from './recipeExtrasService';
import { sortTopMeals } from './sortTopMeals';
...
const actionTypes = {
  ...
  SET_MY_CHEF_PROFILE: 'SET_MY_CHEF_PROFILE',
...
const appStateReducer = (state, action) => {
  switch (action.type) {
    ...
    case actionTypes.SET_MY_CHEF_PROFILE:
      return { ...state, myChefProfile: action.payload };
    ...
    case actionTypes.UPDATE_SINGLE_RESPONSE:
      ...
      const updatedResponses = { ...existingCache.responses };
...
export const AppStateProvider = ({ children }) => {
  ...
  const checkDateChange = useCallback(async () => {
    ...
    if (lastCheckedDate.current !== today) {
      ...
      try {
        // Clear AsyncStorage daily responses (prevents stale data on next app launch)
        await AsyncStorage.removeItem(STORAGE_KEYS.CACHED_DAILY_RESPONSES);
        log.cache('Cleared AsyncStorage daily responses cache');

        // Clear all in-memory caches to force fresh data
        dispatch({ type: actionTypes.INVALIDATE_CACHE, payload: ['all'] });

        // Trigger a refresh so UI updates immediately
  ...
  useEffect(() => {
    if (!midnightResetPerformed.current) {
      ...
      log.cache('Checking for date change on app startup...');
      checkDateChange();
    }
  }, [checkDateChange]);

  // Schedule a timer for the next midnight so responses reset even if the app stays open
  useEffect(() => {
    const scheduleMidnightTimer = () => {
      ...
      log.cache('Scheduling midnight reset timer in', Math.round(msUntilMidnight / 1000 / 60), 'minutes');
      return setTimeout(() => {
        log.cache('Midnight timer fired!');
        checkDateChange();
        // Schedule the next one (for apps that stay open multiple days)
  ...
  // Listen for app returning to foreground (e.g. after overnight background)
  // This ensures daily responses are reset after midnight
  const appStateRef = useRef(AppState.currentState);
  useEffect(() => {
    const subscription = AppState.addEventListener('change', (nextAppState) => {
      if (appStateRef.current.match(/inactive|background/) && nextAppState === 'active') {
        log.cache('App returned to foreground, checking for date change...');
        checkDateChange();
      }
  ...
  const loadCachedDailyResponses = useCallback(async () => {
    ...
    // If in-memory cache exists, verify it's from today before using it
    if (state.dailyResponsesCache && Object.keys(state.dailyResponsesCache).length > 0) {
      const anyGroupCache = Object.values(state.dailyResponsesCache)[0];
      if (anyGroupCache?.date === today) {
        log.cache('Skipping cached responses - already have today\'s responses in cache');
        return state.dailyResponsesCache;
      }
      // In-memory cache is from a previous day — clear it
      log.cache('In-memory daily responses are stale (from', anyGroupCache?.date, '), clearing...');
      dispatch({ type: actionTypes.INVALIDATE_CACHE, payload: ['dailyResponses'] });
    }
    ...
    try {
      ...
      if (cached) {
        ...
        if (data.date === today) {
          ...
          log.cache('Cached daily responses are from a different day, removing...');
          await AsyncStorage.removeItem(STORAGE_KEYS.CACHED_DAILY_RESPONSES);
        }
      }
    } catch (error) {
  ...
  const loadTopMeals = useCallback(async (requestId, force = false) => {
    ...
    try {
      ...
      if (result.success) {
        const sorted = sortTopMeals(result.topMeals);
        dispatch({ 
          type: actionTypes.SET_TOP_MEALS, 
          payload: { requestId, meals: sorted } 
        });
        return sorted;
      }
      return [];
  ...
  const loadDinnerRequests = useCallback(async (force = false) => {
    ...
    if (!force && isCacheFresh(state.dinnerRequestsLastUpdated)) {
  ...
  // Clear ALL cached data (AsyncStorage + in-memory) — used on logout
  const clearAllCachedData = useCallback(async () => {
    log.cache('Clearing ALL cached data for logout...');
    
    // Wipe all AsyncStorage keys
    try {
      await AsyncStorage.multiRemove(Object.values(STORAGE_KEYS));
      log.cache('AsyncStorage cleared');
    } catch (error) {
      debugError('CACHE', 'Error clearing AsyncStorage:', error);
    }
    
    // Reset in-memory state to initial
    dispatch({ type: actionTypes.RESET_ALL });
    initialLoadPerformed.current = false;
    profileLoadedRef.current = false;
    
    log.cache('All cached data cleared');
  }, []);
  ...
  const fastLoadDashboard = useCallback(async () => {
    ...
    try {
      ...
      if (result.profile) {
        ...
        try {
          await AsyncStorage.setItem(STORAGE_KEYS.CACHED_PROFILE, JSON.stringify(result.profile));
        } catch (e) {}
      }
      ...
      const groups = (result.groups || []).map(g => ({
        ...
        member_count: g.member_count ?? g.members_count ?? g.member_total ?? g.members?.length ?? 0
      }));
  ...
  useEffect(() => {
    if (!state.isGuest && !initialLoadPerformed.current) {
      ...
      (async () => {
        ...
        // Load recipe extras (servings + cost) in background
        loadRecipeExtras().catch(() => {});

        // Notification permission is requested from MainTabNavigator after splash disappears
      })();
    }
  ...
  const value = {
    ...
    // Chef profile
    myChefProfile: state.myChefProfile,
    setMyChefProfile: (chef) => dispatch({ type: actionTypes.SET_MY_CHEF_PROFILE, payload: chef }),
