        ...
        Object.entries(result.preloadedData).forEach(([groupId, data]) => {
          ...
          if (data.mealRequest) {
            ...
            if (data.topMeals?.length > 0) {
              dispatch({ 
                type: actionTypes.SET_TOP_MEALS, 
                payload: { requestId: data.mealRequest.id, meals: data.topMeals } 
              });
            }
          }
        });
    ...
    try {
      ...
      (result.groups || []).forEach(g => {
        ...
        preloadedData[groupId] = {
          ...
          mealRequest: g.active_meal_request || null,
          topMeals: g.top_meals || [],
          recipeType: g.active_meal_request?.recipe_type || null,
          timestamp: timestamp
        };
        ...
        if (g.active_meal_request) {
          ...
          if (g.top_meals?.length > 0) {
            dispatch({ 
              type: actionTypes.SET_TOP_MEALS, 
              payload: { requestId: g.active_meal_request.id, meals: g.top_meals } 
            });
          }
        }
      });
