...
export const shareRecipeWithGroups = async (recipeOrId, groupIds, opts = {}) => {
  ...
  // When the id isn't a uuid, the recipe lives only as a snapshot — we write
  // it to recipe_group_shares with recipe_id: null and match on the JSONB id.
  const idIsUuid = isUuid(recipeId);

  try {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return { success: false, error: 'Not authenticated' };

    // Fetch existing shares so we can diff. Query path depends on whether
    // we have a real FK or a snapshot-only id.
    let existingQuery = supabase
      .from('recipe_group_shares')
      .select('id, group_id, recipe_data')
      .eq('shared_by', user.id);
    if (idIsUuid) {
      existingQuery = existingQuery.eq('recipe_id', recipeId);
    } else {
      existingQuery = existingQuery.filter('recipe_data->>id', 'eq', String(recipeId));
    }
    ...
    if (toRemove.length > 0) {
      ...
      if (idIsUuid) {
        delQuery = delQuery.eq('recipe_id', recipeId);
      } else {
        delQuery = delQuery.filter('recipe_data->>id', 'eq', String(recipeId));
      }
    ...
    if (toAdd.length > 0) {
      ...
      const rows = toAdd.map((gid) => ({
        // Snapshot-only ids can't be stored in the uuid FK; keep it null.
        recipe_id: idIsUuid ? recipeId : null,
        group_id: gid,
        shared_by: user.id,
...
/**
 * True iff the given value looks like a UUID.
 * Needed because recipe_group_shares.recipe_id is a uuid column — matching
 * against a non-uuid snapshot id crashes the Postgres cast.
 */
const isUuid = (v) =>
  typeof v === 'string' &&
 ...
 *
 * Accepts either a real recipes.id uuid OR a snapshot id (string).
 * Optionally accepts a shareId — the row's own primary key — which is the
 * most reliable path when both the FK and the snapshot id are ambiguous.
 */
export const removeRecipeFromGroup = async (recipeId, groupId, shareId = null) => {
  if (!USE_REAL_SUPABASE) return { success: true };
  if (!groupId && !shareId) return { success: false, error: 'groupId or shareId required' };
  try {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return { success: false, error: 'Not authenticated' };

    let query = supabase.from('recipe_group_shares').delete();

    if (shareId) {
      query = query.eq('id', shareId);
    } else if (isUuid(recipeId)) {
      // Real FK OR snapshot id that happens to match the FK.
      query = query.eq('group_id', groupId).or(
        `recipe_id.eq.${recipeId},recipe_data->>id.eq.${String(recipeId)}`
      );
    } else {
      // Snapshot-only delete. Passing a non-uuid into recipe_id.eq would
      // crash the Postgres cast, so we narrow to the JSONB id match only.
      query = query
        .eq('group_id', groupId)
        .filter('recipe_data->>id', 'eq', String(recipeId));
    }
 ...
 * count as "this recipe is in this group". Non-UUID ids are routed to the
 * snapshot-only path so the Postgres uuid cast never fires.
 */
export const getRecipeShares = async (recipeId) => {
  ...
  try {
    let query = supabase.from('recipe_group_shares').select('group_id');
    if (isUuid(recipeId)) {
      query = query.or(
        `recipe_id.eq.${recipeId},recipe_data->>id.eq.${String(recipeId)}`
      );
    } else {
      query = query.filter('recipe_data->>id', 'eq', String(recipeId));
    }
    const { data, error } = await query;
