...
export const getRandomRecipes = async (limit = 20) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('recipes')
      .select('id, name, description, image, cooking_time_minutes, cuisine_type, ingredients, steps, chef_id, chef_profiles(id, name, tag, profile_image, description, links)')
      .limit(fetchCount);
