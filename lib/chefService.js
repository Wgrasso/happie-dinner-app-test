...
export const ensureChefProfile = async () => {
  ...
  try {
    ...
    // Slug-ify the user's first name. The tag should just look like their
    // name (e.g. "willem") — no random suffix. We only append a number if
    // the clean slug is already taken by another user.
    const baseTag =
      firstName
        .toLowerCase()
        .normalize('NFD')
    ...
    // Walk 'base', 'base2', 'base3', … until we find an unused tag.
    let tag = baseTag;
    for (let i = 2; i <= 50; i++) {
      const available = await isTagAvailable(tag);
      if (available) break;
      tag = `${baseTag}${i}`;
    }
