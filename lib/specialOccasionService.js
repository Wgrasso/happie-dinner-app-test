...
export const deleteSpecialOccasion = async (occasionId) => {
  ...
  try {
    ...
    const { data: rpcRaw, error: rpcErr } = await supabase.rpc('delete_special_occasion', {
      p_occasion_id: occasionId,
    });

    // Supabase may return the JSON result as a string — normalise it
    ...
    const rpcNotFound = rpcErr && (rpcErr.message?.includes('function') || rpcErr.code === '42883');

    if (rpcNotFound) {
      console.warn('[OCCASION] delete RPC not found, using direct delete');
    } else if (rpcErr) {
      console.error('[OCCASION] RPC error:', rpcErr.message);
      // Don't bail out — try direct delete as fallback
