...
export const buildInviteMessage = (code) => {
  // Query-string form (?code=XXX) so the URL works on every static
  // host without requiring a path-rewrite rule. The page at
  // /join/ reads the code from the query string and can still hand
  // off to the app via AASA (iOS) or deep link (tap through).
  const groupLink = `https://studentenhappie.nl/join/?code=${encodeURIComponent(code)}`;
  return (
    `🍽 Je bent uitgenodigd voor een groep op Happie!\n\n` +
    `Groepscode: ${code}\n\n` +
    `Download de app hier:\n` +
    `https://apps.apple.com/app/happie/id6757129676\n\n` +
    `Open de app → "Groep Joinen" → vul de code in.\n\n` +
    `Of open de groep direct:\n` +
    `${groupLink}`
  );
};
