...
import { strakOrangeTheme } from '../themes/strak-orange';
...
const originalTheme = {
  ...
  colors: {
    ...
    text: '#2D2D2D',
    // textSecondary previously collided with `primary` (#8B7355), flattening
    // every hierarchy in the app. Split into a calmer muted tone so body copy,
    // captions, and helper text visually recede from buttons and links.
    textSecondary: '#7A6B57',
    textTertiary: '#9B8E7E',
    textPlaceholder: '#8A7E6F',
    textInverse: '#FFFFFF',
...
export const ThemeProvider = ({ children }) => {
  const value = useMemo(() => ({
    theme: strakOrangeTheme,
    themeKey: 'strak-orange',
    switchTheme: () => {},
    themeNames: ['strak-orange'],
  }), []);
...
export const useTheme = () => {
  ...
  if (!ctx) return { theme: strakOrangeTheme, themeKey: 'strak-orange', switchTheme: () => {}, themeNames: ['strak-orange'] };
  return ctx;
};
