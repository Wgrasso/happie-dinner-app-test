...
export const getUserGroups = async () => {
  ...
  try {
    ...
    const { data: groups, error: groupsError } = await supabase
      .from('groups')
      .select('id, name, description, join_code, created_by, is_active, created_at, is_main_group, photo_url')
      .in('id', groupIds)
      .eq('is_active', true)
...
export const leaveGroup = async (groupId) => {
  ...
  try {
    ...
    const wasCreator = group?.created_by === user.id;
    ...
    // Transfer admin to longest-standing member
    if (wasCreator) {
      const newAdmin = remaining[0].user_id;
      await supabase.from('groups').update({ created_by: newAdmin }).eq('id', groupId);
      await supabase.from('group_members').update({ role: 'admin' }).eq('group_id', groupId).eq('user_id', newAdmin);
    }
