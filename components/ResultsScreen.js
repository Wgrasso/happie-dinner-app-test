...
export default function ResultsScreen({ route, navigation }) {
  ...
  // IMPORTANT: Do NOT use navigation.navigate('MainTabs', params) - it creates a NEW
  // MainTabs instance instead of returning to the existing one, losing all state.
  ...
  const getPositionText = (index) => {
    // Dutch ordinal formatting
    const rank = index + 1;
    // Special cases for Dutch ordinals
    if (rank === 1) return t('voting.place_1');
    if (rank === 2) return t('voting.place_2');
    if (rank === 3) return t('voting.place_3');
    return t('voting.place', { rank: `${rank}` });
  };
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          ...
          >
            <Text style={styles.backButtonText}>← {t('meals.backToGroup')}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          ...
          <Text style={styles.headerSubtext}>{t('voting.results')}</Text>
        </View>
      ...
      >
        ...
        <View style={styles.titleSection}>
          <Text style={styles.resultsTitle}>{t('voting.topVotedMeals')}</Text>
          <Text style={styles.resultsSubtitle}>
            {t('voting.groupFavorites')}
          </Text>
        </View>
        ...
        {topMeals.length > 0 && (
          <View style={styles.summarySection}>
            <Text style={styles.summaryTitle}>{t('voting.summary')}</Text>
            <Text style={styles.summaryText}>
              {t('voting.summary_active', { count: topMeals.length })}
            </Text>
          </View>
        )}
      </ScrollView>
